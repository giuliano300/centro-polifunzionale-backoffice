...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, MatButtonModule, MatCardModule, MatFormFieldModule, MatInputModule, MatSlideToggleModule, MatTableModule],
...
export class CourseTagsComponent implements OnInit {
  ...
  tags: CourseTagOption[] = []; editingId: string | null = null; message = ''; isSaving = false; editorOpen = false;
  ...
  add() { this.editingId = null; this.message = ''; this.form.reset({ value: '', label: '', sortOrder: 0, isActive: true }); this.editorOpen = true; }
  edit(tag: CourseTagOption) {
    this.editingId = tag._id || null;
    this.message = '';
    this.form.setValue({ value: tag.value, label: tag.label, sortOrder: tag.sortOrder || 0, isActive: tag.isActive !== false });
    this.editorOpen = true;
    setTimeout(() => document.querySelector('.tag-editor-card')?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  }
  cancel() { this.editingId = null; this.editorOpen = false; this.form.reset({ value: '', label: '', sortOrder: 0, isActive: true }); }
  save() {
  ...
  remove(tag: CourseTagOption) {
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      width: '480px',
      maxWidth: '92vw',
      data: { title: 'Eliminare questo tag?', message: 'Il tag non sarà più disponibile per corsi e interessi.', detail: tag.label }
    });
