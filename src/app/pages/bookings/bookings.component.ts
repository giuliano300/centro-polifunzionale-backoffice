...
import { closeMobileSheet } from '../../core/mobile-sheet';
...
export class BookingsComponent {
  ...
  load(): void {
    ...
    this.api.bookings().subscribe({
      next: (items) => {
        ...
        const requestedBooking = this.requestedBookingId
          ? items.find((item) => item.booking._id === this.requestedBookingId)
          : undefined;
        if (requestedBooking) {
          const selectedDate = new Date(requestedBooking.booking.date);
          selectedDate.setHours(0, 0, 0, 0);
          this.selectedCalendarDate = selectedDate;
          this.selectedCalendarBooking = requestedBooking;
        } else {
          this.syncSelectedCalendarBooking();
        }
        this.isLoading = false;
        setTimeout(() => {
          if (requestedBooking && this.calendar) {
            this.calendar.activeDate = this.selectedCalendarDate || this.today();
          }
          this.calendar?.updateTodaysDate();
          if (requestedBooking) {
            document.getElementById('calendar-bookings-summary')?.scrollIntoView({
              behavior: 'smooth',
              block: 'start',
            });
          }
  ...
  selectedBookingRentalLabel(item: BookingWithPayments): string {
    ...
    if (space?.sectorEnabled && item.booking.sectorQuantity && item.booking.sectorQuantity < Number(space.sectorCount || 1)) {
      return item.booking.sectorQuantity === 1 ? '1 area' : `${item.booking.sectorQuantity} aree`;
    }

    return 'Stanza intera a frazioni';
  }

  bookingSectorLabel(item: BookingWithPayments): string {
    const space = typeof item.booking.space === 'string' ? null : item.booking.space;
