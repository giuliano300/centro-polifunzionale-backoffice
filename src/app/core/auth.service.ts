...
export class AuthService {
  ...
  socialSignIn(provider: 'google' | 'facebook', idToken: string, name?: string): Observable<any> {
    return this.http.post<any>(`${environment.apiUrl}auth/social/sign-in`, { provider, idToken, name }).pipe(
      tap((response) => this.acceptAccessToken(response)),
    );
  }

  requestSocialPhoneOtp(completionToken: string, phone: string): Observable<any> {
    return this.http.post<any>(`${environment.apiUrl}auth/social/request-phone-otp`, { completionToken, phone });
  }

  completeSocialRegistration(payload: {
    ...
    return this.http.post<any>(`${environment.apiUrl}auth/social/complete-registration`, payload).pipe(
      tap((response) => this.acceptAccessToken(response)),
    );
  }
  ...
  isAdmin(): boolean {
    return this.payload()?.role === UserRole.Admin;
  }
  ...
  private acceptAccessToken(response: any): void {
