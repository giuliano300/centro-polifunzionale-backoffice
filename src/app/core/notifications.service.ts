...
export class NotificationsService {
  ...
  connect(): void {
    ...
    if (this.socket?.connected && this.connectedUserId === userId) return;
  ...
  onChatMessage(handler: (message: any) => void): void { this.socket?.on('course-chat-message', handler); }
  offChatMessage(handler: (message: any) => void): void { this.socket?.off('course-chat-message', handler); }

  enablePush(): void {
    const token = this.auth.token();
    if (!token || !('serviceWorker' in navigator) || !('PushManager' in window) || Notification.permission === 'denied') return;
    this.http.get<{ publicKey: string }>(`${environment.apiUrl}notifications/push/public-key`).subscribe(({ publicKey }) => {
      if (!publicKey) return;
      navigator.serviceWorker.register('/push/sw.js', { scope: '/push/' }).then(async (registration) => { await registration.update(); return registration.pushManager.getSubscription().then((existing) => existing || registration.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: this.vapidKey(publicKey) })); }).then((subscription) => {
        this.http.post(`${environment.apiUrl}notifications/push/subscribe`, subscription.toJSON(), { headers: { Authorization: `Bearer ${token}` } }).subscribe();
      }).catch(() => undefined);
    });
  }

  private vapidKey(value: string): Uint8Array<ArrayBuffer> {
